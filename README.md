...
**voxigo** builds real-time voice agents in Go: audio in over WebRTC, a streaming
...
voxigo stays on plain, standard WebRTC via [Pion](https://github.com/pion) — no
...
voxigo uses cgo (`CGO_ENABLED=0` is not supported) and a few native libraries:
...
**With Docker** — voxigo publishes a build base and a distroless runtime base
