...
voxigo's pipeline/bench_test.go.
...
architecture itself, where voxigo (goroutines) and Pipecat (a single asyncio
...
plumbing. Run voxigo's side with:
...
class Passthrough(FrameProcessor):
    ...
    voxigo's passthrough. Isolates per-hop plumbing cost from real processing."""
...
async def bench_concurrency(results: dict) -> None:
    ...
    on the one event loop via gather; this is the ceiling voxigo's goroutines are
