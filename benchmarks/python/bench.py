"""Pipecat-side micro-benchmarks, for comparison against voxigo's Go benchmarks.
...
The same ONNX model files voxigo embeds are loaded here, so only the surrounding
code differs. Run voxigo's side with:
...
def gen_audio(n: int) -> np.ndarray:
    """The same deterministic signal voxigo's benchmarks use."""
    i = np.arange(n)
    return (
