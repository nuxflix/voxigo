...
const (
	...
	defaultChannels = 1
	// defaultEndpointingMs is voxigo's turn-detection default: it is the silence
	...
	// end a turn on every micro-pause, so voxigo keeps this rather than leave it
...
type Config struct {
	...
	// voxigo's turn-taking uses its own Silero VAD, not Deepgram's events.
	VADEvents *bool
	...
	// the param. voxigo does not consume Deepgram's UtteranceEnd events, so this is
