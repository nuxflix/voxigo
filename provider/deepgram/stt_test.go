...
// opinionated Deepgram params voxigo used to force are omitted when unset, while
// endpointing stays set because voxigo gates turn detection on speech_final.
func TestQueryDefaultsMatchPipecat(t *testing.T) {
	...
	// endpointing is voxigo's turn-detection gate, so it stays set.
