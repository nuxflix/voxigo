...
| --- | --- | --- |
| Whisper log-mel feature extraction | **Fair language comparison** | Same math, voxigo in Go vs Pipecat in numpy. |
...
Both sides load the **same ONNX model files** (the ones voxigo embeds), so the
...
- **Silero VAD: a tie.** Both bind the same ONNX Runtime, and the per-frame glue
  is negligible. This is the common case for anything model-backed: voxigo neither
...
- **Feature extraction: voxigo is currently slower.** numpy's `rfft` is an
  `O(N log N)` FFT with vectorized SIMD; voxigo's feature extractor uses a naive
...
model-backed work, voxigo and Pipecat call the same runtime. The wins are
...
- **Low, predictable memory.** No interpreter heap or numpy/torch resident set;
  a voxigo agent's footprint is dominated by the ONNX model, not the runtime.
...
voxigo trades numpy's tuned numerics — recoverable with a better FFT — for these
