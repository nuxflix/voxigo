# Benchmarks: voxigo vs Pipecat

voxigo is a Go port of [Pipecat](https://github.com/pipecat-ai/pipecat). A natural
...
2. **Pipeline plumbing & concurrency** — the frame-transport architecture, where
   voxigo (goroutines) and Pipecat (one asyncio event loop) genuinely differ.
...
```sh
# voxigo (Go) — turn-taking
VOXIGO_ONNXRUNTIME_LIB=/path/to/libonnxruntime.so \
  go test -run '^$' -bench 'ComputeLogMel|Silero|SmartTurnPredict' -benchmem \
  ./audio/turn/ ./audio/vad/

# voxigo (Go) — pipeline plumbing & concurrency
...
| Component | Pipecat (Python) | voxigo (Go) | |
| --- | ---: | ---: | --- |
| Whisper log-mel features (2 s utterance) | ~41 ms | ~30 ms | **voxigo ~1.4× faster** |
| Silero VAD (per 32 ms frame) | ~0.63 ms | ~0.73 ms | tie |
| Smart Turn, end-to-end (features + ONNX) | ~210 ms | ~222 ms | tie |

- **Feature extraction — voxigo wins.** This is the one true language comparison:
  the same math, voxigo in Go vs Pipecat in numpy. voxigo computes the 400-point
...
- **Silero VAD — a tie.** Both bind the same ONNX Runtime; the per-frame glue is
  negligible. This is the common case for anything model-backed: voxigo neither
...
| Chain depth | Pipecat (Python) | voxigo (Go) |
| --- | ---: | ---: |
...
voxigo's per-hop cost is goroutine wakeup latency; Pipecat's is an `await`
through each processor's asyncio queue plus its worker bookkeeping. voxigo is
~15–40× faster per hop.

**Aggregate throughput** as simultaneous sessions scale (4-processor chain):

| Sessions | Pipecat (Python) | voxigo (Go) |
| --- | ---: | ---: |
...
under scheduling overhead, and 1000 sessions did not complete in 280 s). voxigo's
...
- voxigo now **wins the one fair compute comparison** (feature extraction), after
  replacing a naive DFT and a dense mel projection with an FFT and a sparse,
  cache-friendly filterbank.
- The decisive win is **concurrency**: a single voxigo process scales many
