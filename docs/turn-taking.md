...
voxigo that uses cgo. The runtime shared library is **not** bundled; download a
...
- Turn analysis currently runs synchronously on the audio goroutine
  (~tens of ms per end-of-turn). See [benchmarks](https://github.com/nuxflix/voxigo-benchmarks) for the
