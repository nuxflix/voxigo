// Command voice-rime is a headless voxigo voice backend.
//
// It uses Rime text-to-speech, with Deepgram STT and an Anthropic LLM.
//
...
func runBot(conn *pionrtc.Connection) {
	...
	tts := rime.NewTTS(rime.Config{APIKey: os.Getenv("RIME_API_KEY")})
	// ----------------------------------------------------------------------
