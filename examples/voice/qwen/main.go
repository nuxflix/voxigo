// Command voice-qwen is a headless voxigo voice backend.
//
// It uses an Alibaba Qwen LLM (DashScope, OpenAI-compatible), with Deepgram STT and ElevenLabs TTS.
//
...
func runBot(conn *pionrtc.Connection) {
	...
	llm := qwen.NewLLM(openai.LLMConfig{APIKey: os.Getenv("DASHSCOPE_API_KEY")})
