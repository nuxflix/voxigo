// Command voice-gladia is a headless voxigo voice backend.
//
// It uses Gladia speech-to-text, with an Anthropic LLM and ElevenLabs TTS.
//
...
func runBot(conn *rtc.Connection) {
	...
	stt := gladia.NewSTT(gladia.Config{APIKey: os.Getenv("GLADIA_API_KEY"), SampleRate: opus.SampleRate})
