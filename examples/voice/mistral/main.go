// Command voice-mistral is a headless voxigo voice backend.
//
// It uses a Mistral LLM (OpenAI-compatible), with Deepgram STT and ElevenLabs TTS.
//
...
func runBot(conn *pionrtc.Connection) {
	...
	llm := mistral.NewLLM(openai.LLMConfig{APIKey: os.Getenv("MISTRAL_API_KEY")})
