// Command voice-hume is a headless voxigo voice backend.
//
// It uses Hume Octave text-to-speech (set HUME_VOICE_ID), with Deepgram STT and an Anthropic LLM.
//
...
func runBot(conn *pionrtc.Connection) {
	...
	tts := hume.NewTTS(hume.Config{APIKey: os.Getenv("HUME_API_KEY"), VoiceID: os.Getenv("HUME_VOICE_ID")})
	// ----------------------------------------------------------------------
