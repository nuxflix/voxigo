// Command voice-lmnt is a headless voxigo voice backend.
//
// It uses LMNT text-to-speech, with Deepgram STT and an Anthropic LLM.
//
...
func runBot(conn *rtc.Connection) {
	...
	tts := lmnt.NewTTS(lmnt.Config{APIKey: os.Getenv("LMNT_API_KEY")})
	// ----------------------------------------------------------------------
