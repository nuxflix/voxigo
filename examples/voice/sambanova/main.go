// Command voice-sambanova is a headless voxigo voice backend.
//
// It uses a SambaNova LLM (OpenAI-compatible), with Deepgram STT and ElevenLabs TTS.
//
...
//
//	DEEPGRAM_API_KEY=… SAMBANOVA_API_KEY=… ELEVENLABS_API_KEY=… go run ./examples/voice/sambanova
package main
...
func runBot(conn *pionrtc.Connection) {
	...
	llm := sambanova.NewLLM(openai.LLMConfig{APIKey: os.Getenv("SAMBANOVA_API_KEY")})
