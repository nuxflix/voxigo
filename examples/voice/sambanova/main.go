// Command voice-sambanova is the voice bot with a SambaNova LLM, served through
...
func main() {
	if err := run.Voice(run.Builders{
		STT: run.DefaultSTT,
		LLM: run.Service(openai.LLMConfig{APIKey: os.Getenv("SAMBANOVA_API_KEY")}, sambanova.NewLLM),
		TTS: run.DefaultTTS,
	}); err != nil {
		log.Fatal(err)
	}
