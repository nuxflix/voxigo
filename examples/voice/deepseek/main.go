// Command voice-deepseek is a headless voxigo voice backend.
//
// It uses a DeepSeek LLM (OpenAI-compatible), with Deepgram STT and ElevenLabs TTS.
//
...
func runBot(conn *pionrtc.Connection) {
	...
	llm := deepseek.NewLLM(openai.LLMConfig{APIKey: os.Getenv("DEEPSEEK_API_KEY")})
