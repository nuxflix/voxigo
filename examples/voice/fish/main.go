// Command voice-fish is a headless voxigo voice backend.
//
...
func runBot(conn *pionrtc.Connection) {
	...
	tts := fish.NewTTS(fish.Config{APIKey: os.Getenv("FISH_API_KEY"), ReferenceID: os.Getenv("FISH_REFERENCE_ID")})
	// ----------------------------------------------------------------------
