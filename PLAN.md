...
voxigo lets you build a real-time voice agent: audio comes in over WebRTC, flows
...
- **RTVI** on the data channel, so existing RTVI clients (web, iOS, Android)
  interoperate with a voxigo server out of the box.
...
transport- and modality-agnostic. voxigo ships exactly one transport (Pion
...
Decision: voxigo **consolidates Pipecat's `RTVIProcessor` + `RTVIObserver` into a
...
- [x] `internal/onnxrt`: the single cgo boundary — locates `libonnxruntime`
      (via `VOXIGO_ONNXRUNTIME_LIB`), initializes the runtime once, and creates
...
VAD gating is **confidence-only** — voxigo trusts the neural model rather than
porting Pipecat's EBU R128 volume gate. voxigo ships **one opinionated path**
...
- **ONNX runtime distribution** — *resolved:* models are embedded with
  ...
  `VOXIGO_ONNXRUNTIME_LIB` env var (`internal/onnxrt`).
